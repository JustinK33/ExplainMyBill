            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2 text-base font-semibold text-white">
                  <ChevronsRight className="h-5 w-5 text-[var(--accent-2)]" />
                  <span className="break-words">{item.label}</span>
                </div>
                <div className="mt-2 text-base leading-7 text-[var(--muted)]">{item.original_text}</div>
              </div>
              <div
              ...
              >
                {item.flagged ? <AlertTriangle className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}
                {item.flagged ? 'Flagged' : 'Looks normal'}
              </div>
            </div>
            <div className="mt-4 rounded-2xl border border-white/[0.08] bg-black/20 p-4 text-base leading-8 text-white">
              {item.plain_english}
            </div>
            {item.flagged && item.flag_reason ? (
              <div className="mt-3 rounded-2xl border border-[var(--danger)]/30 bg-[rgba(255,107,95,0.08)] px-4 py-3 text-base leading-7 text-[#ffd5d1]">
                {item.flag_reason}
              </div>
            ) : null}
          </div>
